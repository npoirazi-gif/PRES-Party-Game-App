...
function GameCard({ game, index, onPress }: { game: Game; index: number; onPress: () => void }) {
  ...
  return <Pressable onPress={onPress} style={({ pressed }) => [styles.card, pressed && styles.pressed]}>
    <View style={styles.sticker}>{gameImages[game.id] ? <Image source={gameImages[game.id]} style={styles.stickerImg} resizeMode="contain" /> : <View style={[styles.stickerShape, { backgroundColor: game.color }]}><Feather name="zap" size={34} color="#140B4A" /></View>}</View>
