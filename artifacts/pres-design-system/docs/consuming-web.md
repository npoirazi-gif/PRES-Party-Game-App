# Consuming PRES Design System in web apps

Read `artifacts/pres-design-system/docs/AGENTS.md` first. This guide covers
...
`artifacts/pres-design-system/docs/migrating-web.md` before writing UI.
...
```css
@import "@workspace/pres-design-system/styles.css";
```
...
`node_modules/@workspace/pres-design-system/src/components` to `content`.
...
```tsx
import { Button } from "@workspace/pres-design-system/components/ui/button";
import { cn } from "@workspace/pres-design-system/lib/utils";
import {
  toast,
  useToast,
} from "@workspace/pres-design-system/hooks/use-toast";
```
...
## Dark mode (required for PRES)

PRES is a dark-first product. The token CSS variables are split into `:root`
(light) and `.dark` (dark). The PRES dark palette activates when a `.dark`
class is present on a parent element — typically `<html>`.

Add it to your app's entry HTML:
...
The design system preview (`index.html`) already ships with `class="dark"` on
`<html>` so the style guide always shows the PRES nightlife palette by default.

## Verify

After wiring the workspace dependency, import and render
`@workspace/pres-design-system/components/ui/button`. Run the app's typecheck
and dev server. The import must resolve and the Button must render on a dark
(`#140B4A`) background before broader UI work begins.
